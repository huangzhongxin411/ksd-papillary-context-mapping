...
stage_id <- "stage7E_figure5_targeted_polish_v0.5"
...
log_path <- file.path(log_dir, "stage7E_figure5_targeted_polish_v0.5.log")
...
pB <- ggplot(panelB, aes(x = x_hires, y = y_hires_plot)) +
  ...
  labs(title = "Representative papillary projection",
       subtitle = "Visualization only; not lesion localization",
       caption = "GSM6250309", x = NULL, y = NULL) +
  ...
  theme(
    ...
    plot.caption = element_text(size = 7.8, color = muted, hjust = 0),
    plot.margin = margin(3, 4, 4, 5)
  )
...
pC <- ggplot(panelC, aes(x = context_display, y = module_display,
  ...
  geom_text(aes(label = cell_label), family = font_family, size = 3.38,
  ...
  theme(axis.text.x = element_text(angle = 20, hjust = 1),
        ...
        plot.caption = element_text(size = 7.9, color = muted, hjust = 0, margin = margin(t = 3)),
...
pE <- ggplot() +
  ...
  geom_label(data = boundary_pills, aes(x = x, y = y, label = label),
             ...
             label.padding = unit(0.13, "lines"), label.r = unit(0.18, "lines")) +
...
pF <- ggplot() +
  ...
  annotate("text", x = 0.25, y = 0.40,
           ...
           family = font_family, fontface = "bold", size = 2.82,
...
pdf_path <- file.path(fig_dir, "figure5_spatial_twas_context_draft_v0.5.pdf")
png_path <- file.path(fig_dir, "figure5_spatial_twas_context_draft_v0.5.png")
svg_path <- file.path(fig_dir, "figure5_spatial_twas_context_draft_v0.5.svg")
...
fwrite(manifest, file.path(table_dir, "figure5_source_data_manifest_v0.5.tsv"), sep = "\t")

changes <- data.table(
  panel = c("overall", LETTERS[1:6]),
  issue_in_v0.4 = c(
    "Accepted layout required final production polish",
    "No-ROI boundary read as a plain text bar",
    "Spatial overlays could use slightly more image area",
    "Cell labels and status key needed stronger scale readability",
    "Facets and median trajectory needed modest emphasis",
    "Boundary notes remained table-like",
    "Boundary content retained a checklist rhythm"
  ),
  polish_made_in_v0.5 = c(
    "Preserved evidence-weighted geometry and refined typography",
    "Converted boundary into a restrained upper-right badge",
    "Reduced internal margins while preserving secondary status",
    "Increased cell and status-key typography slightly",
    "Clarified facet titles, strengthened median, and reduced pair-line alpha",
    "Balanced equal-height cards and replaced stripes with padded pills",
    "Converted lists to compact dot-separated two-zone statements"
  ),
  data_changed = "no",
  claim_changed = "no",
...
fwrite(changes, file.path(table_dir, "figure5_visual_polish_log_v0.5.tsv"), sep = "\t")
