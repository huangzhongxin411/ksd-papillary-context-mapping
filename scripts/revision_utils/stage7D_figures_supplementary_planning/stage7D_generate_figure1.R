...
out_dir <- file.path(root, "results/figures/revision/stage7D_figures_supplementary_planning")
table_dir <- file.path(root, "results/tables/revision/stage7D_figures_supplementary_planning")
log_dir <- file.path(root, "logs/revision/stage7D_figures_supplementary_planning")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(table_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(log_dir, recursive = TRUE, showWarnings = FALSE)

stem <- file.path(out_dir, "figure1_evidence_hierarchy_draft_v0.1")
...
draw_figure <- function() {
  ...
  add_text("Evidence-stratified post-GWAS renal papillary context mapping",
  ...
  add_text("17,316 genes tested", ax + 0.025, ay + 0.052, size = 5.7, hjust = 0)
  add_text("94 Bonferroni genes", ax + 0.025, ay + 0.027, size = 5.7, hjust = 0)
  add_rect(ax + 0.183, ay + 0.066, 0.085, 0.045, fill = pal[["pale_gold"]], col = pal[["gold"]], lwd = 0.7)
  add_text("1000G EUR LD\nreference", ax + 0.183, ay + 0.066, size = 5.2, col = pal[["dark"]], face = "bold")
  ...
  add_text("partial matched-random support", cx + 0.150, cy + 0.037, size = 5.4, col = pal[["grey"]])
  ...
  add_text("MAGMA-prioritized genes map to a\nLoop/TAL-associated papillary context and an\ninjury/remodeling-associated bulk background",
           fx + 0.030, fy + 0.067, size = 5.25, face = "bold", hjust = 0)
...
write.table(panel_source,
            file.path(table_dir, "figure1_panel_source_manifest_v0.1.tsv"),
...
writeLines(log_lines, file.path(log_dir, "stage7D_generate_figure1.log"))
