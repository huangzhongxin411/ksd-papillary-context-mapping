...
stage_id <- "stage7E_figure4_working_final_v0.7"
...
log_path <- file.path(log_dir, "stage7E_figure4_working_final_v0.7.log")
...
pdf_path <- file.path(fig_dir, "figure4_gse73680_bulk_context_working_final_v0.7.pdf")
png_path <- file.path(fig_dir, "figure4_gse73680_bulk_context_working_final_v0.7.png")
svg_path <- file.path(fig_dir, "figure4_gse73680_bulk_context_working_final_v0.7.svg")
...
fwrite(manifest, file.path(table_dir, "figure4_source_data_manifest_v0.7.tsv"), sep = "\t")

changes <- data.table(
  panel = c("overall", "A", "B", "C", "D", "E", "F", "overall", "overall"),
  issue_in_v0.6 = c("Working-final production pass remained", "Arrow and resource strip required final balance", "Violin tone and label angle required final consistency", "CI prominence and FDR alignment required final polish", "Heatmap tone, labels, and colorbar required final refinement", "Whitespace, note hierarchy, and bar height required final polish", "Boundary banner spacing required final polish", "Typography required working-final confirmation", "Palette required working-final confirmation"),
  change_made_in_v0.7 = c("Completed final six-panel alignment pass", "Thickened and centered arrow; raised subtle strip readability", "Lightened violins and standardized 18-degree labels", "Strengthened CI/points, lightened zero line, aligned FDR note", "Reduced saturation, enlarged values, reduced label rotation, tightened colorbar", "Raised bar height, compressed whitespace, centered modest note, tightened headers", "Equalized box height and moved group headings closer", "Confirmed Helvetica hierarchy at full and 50-percent scale", "Confirmed restrained teal/gold/terracotta/gray family"),
  data_changed = "no",
  claim_changed = "no",
...
fwrite(changes, file.path(table_dir, "figure4_micro_polish_change_log_v0.7.tsv"), sep = "\t")
