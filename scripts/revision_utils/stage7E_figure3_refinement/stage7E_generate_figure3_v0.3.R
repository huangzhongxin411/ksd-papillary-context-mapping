...
fig_dir <- file.path(root, "results/figures/revision/stage7E_figure3_refinement")
table_dir <- file.path(root, "results/tables/revision/stage7E_figure3_refinement")
log_dir <- file.path(root, "logs/revision/stage7E_figure3_refinement")
...
# A: conceptual two-axis model.
cells <- data.table(xmin=c(0,1,0,1), xmax=c(1,2,1,2), ymin=c(0,0,1,1), ymax=c(1,1,2,2),
  label=c("Lower priority /\nunresolved context","MAGMA only","TWAS proxy only","MAGMA +\nproxy TWAS"),
  sub=c("reporting context","genetic priority","supplementary proxy","separate evidence axes"),
...
pA <- ggplot(cells)+geom_rect(aes(xmin=xmin,xmax=xmax,ymin=ymin,ymax=ymax,fill=fill),colour=pal[["white"]],linewidth=1.1)+
  geom_text(aes((xmin+xmax)/2,(ymin+ymax)/2+.08,label=label),family=font_family,fontface="bold",size=2.3,lineheight=.9)+
  geom_text(aes((xmin+xmax)/2,(ymin+ymax)/2-.25,label=sub),family=font_family,size=1.65,colour=pal[["grey"]])+
  scale_fill_identity()+annotate("text",x=1,y=2.18,label="Two-axis evidence model\nEvidence separation",family=font_family,fontface="bold",size=1.82,lineheight=.9,colour=pal[["teal"]])+
  scale_x_continuous(breaks=c(.5,1.5),labels=c("Lower / unresolved","MAGMA genetic priority"),limits=c(-.05,2.05),expand=c(0,0))+
  scale_y_continuous(breaks=c(.5,1.5),labels=c("No proxy support","Kidney_Cortex TWAS proxy"),limits=c(-.05,2.28),expand=c(0,0))+
  labs(title="Evidence model concept",x="MAGMA genetic priority",y="Kidney_Cortex TWAS proxy")+theme_pub(7.1)+
  theme(axis.line=element_blank(),axis.ticks=element_blank(),axis.text=element_text(size=5.8),axis.title.x=element_text(face="bold",colour=pal[["teal"]]),
...
group_labels <- c(R1_MAGMA_Bonferroni_only="R1  MAGMA Bonferroni only",
  R2_MAGMA_Bonferroni_plus_multi_snp_TWAS_proxy="R2  Bonferroni + multi-SNP proxy",
  R3_MAGMA_Bonferroni_plus_one_snp_TWAS_proxy="R3  Bonferroni + one-SNP proxy",
  R4_MAGMA_lower_priority="R4  MAGMA lower priority",R5_TWAS_proxy_only="R5  TWAS proxy only",R6_contextual_or_unresolved="R6  Contextual / unresolved")
...
# C: TWAS proxy composition and boundary.
pC <- ggplot()+
  annotate("rect",xmin=0,xmax=42,ymin=.88,ymax=1.20,fill=pal[["coral_pale"]],colour=pal[["white"]],linewidth=.4)+
  annotate("rect",xmin=42,xmax=51,ymin=.88,ymax=1.20,fill=pal[["gold_pale"]],colour=pal[["white"]],linewidth=.4)+
  annotate("text",x=21,y=1.04,label="42  One-SNP",family=font_family,fontface="bold",size=2)+
  annotate("text",x=46.5,y=1.04,label="9\nMulti-SNP",family=font_family,fontface="bold",size=1.75,lineheight=.85)+
  scale_x_continuous(limits=c(0,51),expand=c(0,0))+scale_y_continuous(limits=c(.55,1.78),expand=c(0,0))+
  annotate("text",x=0,y=1.62,label="5,989 tested  ->  51 FDR-supported",hjust=0,family=font_family,fontface="bold",size=2.25,colour=pal[["teal"]])+
  annotate("text",x=0,y=.67,label="Supplementary proxy only  ·  One-SNP models require caution  ·  No papilla-specific eQTL",hjust=0,family=font_family,size=1.72,colour=pal[["grey"]])+
  labs(title="TWAS proxy boundary",subtitle="GTEx Kidney_Cortex proxy",x=NULL,y=NULL)+theme_void(base_family=font_family)+
  theme(plot.title=element_text(face="bold",size=8.1,margin=margin(b=2,l=13)),plot.subtitle=element_text(size=6.8,colour=pal[["gold_dark"]],margin=margin(b=2,l=13)),plot.margin=margin(4,7,4,5))
...
# E: interpretation boundary strip.
supported<-c("Transparent\ncandidate reporting","MAGMA genetic\npriority","Kidney_Cortex proxy\nTWAS context","Interpretive\nexemplars")
not_claimed<-c("Causal genes","Validated genes","SMR / coloc\nsupport","Therapeutic\ntargets")
sx<-c(.075,.225,.375,.525); nx<-c(.66,.77,.88,.985)
pE<-ggplot()+annotate("text",x=.30,y=.90,label="SUPPORTED REPORTING",family=font_family,fontface="bold",size=2,colour=pal[["teal"]])+
  annotate("text",x=.82,y=.90,label="NOT CLAIMED",family=font_family,fontface="bold",size=2,colour=pal[["coral"]])
for(i in seq_along(supported)) pE<-pE+annotate("rect",xmin=sx[i]-.065,xmax=sx[i]+.065,ymin=.20,ymax=.70,fill=if(i==3)pal[["gold_pale"]]else pal[["teal_pale"]],colour=if(i==3)pal[["gold"]]else pal[["teal2"]],linewidth=.38)+
  annotate("text",x=sx[i],y=.45,label=supported[i],family=font_family,fontface="bold",size=1.65,lineheight=.9)
for(i in seq_along(not_claimed)){half<-if(i<3).048 else .050;pE<-pE+annotate("rect",xmin=nx[i]-half,xmax=nx[i]+half,ymin=.20,ymax=.70,fill=pal[["coral_pale"]],colour=pal[["coral"]],linewidth=.38)+
  annotate("text",x=nx[i],y=.45,label=not_claimed[i],family=font_family,fontface="bold",size=1.42,lineheight=.88)}
pE<-pE+coord_cartesian(xlim=c(0,1.025),ylim=c(0,1),clip="off")+labs(title="Claim boundary")+theme_void(base_family=font_family)+
...
fig3<-tag_panel(pA,"A")+tag_panel(pB,"B")+tag_panel(pC,"C")+tag_panel(pD,"D")+tag_panel(pE,"E")+
  ...
  plot_annotation(title="Two-axis candidate evidence model for MAGMA-prioritized genes",subtitle="Genetic priority is separated from Kidney_Cortex proxy TWAS support",
...
width_in<-7.205;height_in<-6.15;stem<-file.path(fig_dir,"figure3_candidate_evidence_model_draft_v0.3")
...
preview_path<-file.path(log_dir,"figure3_candidate_evidence_model_v0.3_qc_50pct.png")
...
log_lines<-c(paste0("timestamp=",format(Sys.time(),"%Y-%m-%dT%H:%M:%S%z")),"backend=R ggplot2/patchwork/svglite/pdf/ragg","figure=Figure 3 v0.3",
  ...
  "source_data_changed_from_v0.2=no","claim_changed_from_v0.2=no",paste0("canvas_inches=",width_in,"x",height_in),"font=Helvetica","png_dpi=600","figure2_modified=no","figure4_started=no")
writeLines(log_lines,file.path(log_dir,"stage7E_generate_figure3_v0.3.log"));cat(paste(log_lines,collapse="\n"),"\n")
