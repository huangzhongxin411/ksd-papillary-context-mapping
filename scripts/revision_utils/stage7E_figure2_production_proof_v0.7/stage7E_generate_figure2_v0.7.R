...
fig_dir <- file.path(root, "results/figures/revision/stage7E_figure2_production_proof_v0.7")
table_dir <- file.path(root, "results/tables/revision/stage7E_figure2_production_proof_v0.7")
log_dir <- file.path(root, "logs/revision/stage7E_figure2_production_proof_v0.7")
...
fwrite(display_mapping, file.path(table_dir, "figure2_display_label_mapping_v0.7.tsv"), sep = "\t")
...
pA <- ggplot() +
  ...
  annotate("segment", x = 0.56, xend = 0.56, y = 0.82, yend = 0.765,
           color = pal[["mid_grey"]], linewidth = 0.46,
           arrow = grid::arrow(length = grid::unit(3.0, "pt"), type = "closed")) +
  annotate("text", x = 0.56, y = 0.72, label = "GSE231569 papilla snRNA",
           family = font_family, fontface = "bold", size = 2.12, color = pal[["deep_teal"]]) +
...
pB <- ggplot(dB, aes(x = donor_display, y = compartment_display, fill = mean_module_score)) +
  ...
  scale_fill_gradientn(
    ...
    name = "Mean score",
    guide = guide_colorbar(barheight = grid::unit(16, "mm"), barwidth = grid::unit(2.7, "mm"),
...
pC <- ggplot() +
  ...
  annotate("text", x = 0.32, y = 0.155, label = "Retained after 5/5 removal panels", hjust = 0,
           family = font_family, size = 1.75, color = pal[["grey"]]) +
  annotate("text", x = 0.02, y = 0.018, label = "Detailed matrices in Source Data", hjust = 0,
           family = font_family, size = 1.5, color = pal[["grey"]]) +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1), clip = "off") +
...
pD <- ggplot(dD, aes(y = module_display)) +
  ...
  annotate("label", x = 119, y = 4.95,
           label = "Observed deltas above\nmatched-random expectation\nOverall: partial support\nRank metrics saturated",
           hjust = 1, family = font_family, fontface = "bold", size = 1.72,
...
width_in <- 7.205
height_in <- 6.45
stem <- file.path(fig_dir, "figure2_snRNA_context_draft_v0.7")
...
if (requireNamespace("magick", quietly = TRUE)) {
  ...
  magick::image_write(half_preview, file.path(log_dir, "figure2_snRNA_context_v0.7_qc_50pct.png"), format = "png")
  ...
  png::writePNG(half_preview, file.path(log_dir, "figure2_snRNA_context_v0.7_qc_50pct.png"))
}

log_lines <- c(
  paste0("timestamp=", format(Sys.time(), "%Y-%m-%dT%H:%M:%S%z")),
  "backend=R ggplot2/patchwork/svglite/pdf/ragg",
  "figure=Figure 2 v0.7",
  ...
  "source_data_changed_from_v0.6=no",
  "claim_changed_from_v0.6=no",
...
writeLines(log_lines, file.path(log_dir, "stage7E_generate_figure2_v0.7.log"))
