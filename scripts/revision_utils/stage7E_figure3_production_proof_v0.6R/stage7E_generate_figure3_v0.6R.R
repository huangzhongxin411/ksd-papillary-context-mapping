...
fig_dir <- file.path(root, "results/figures/revision/stage7E_figure3_production_proof_v0.6R")
table_dir <- file.path(root, "results/tables/revision/stage7E_figure3_production_proof_v0.6R")
log_dir <- file.path(root, "logs/revision/stage7E_figure3_production_proof_v0.6R")
...
pA<-ggplot()+
  ...
  annotate("segment",x=.90,xend=.938,y=.563,yend=.563,colour=pal[["gold"]],linewidth=.46)+
  annotate("text",x=.943,y=.563,label="R2  multi-SNP  1",hjust=0,family=font_family,fontface="bold",size=1.48,colour=pal[["gold_dark"]])+
  annotate("segment",x=.90,xend=.938,y=.886,yend=.886,colour=pal[["gold"]],linewidth=.46)+
  annotate("text",x=.943,y=.886,label="R5  proxy only  4",hjust=0,family=font_family,fontface="bold",size=1.48,colour=pal[["gold_dark"]])+
...
pB <- ggplot(waffle,aes(x,y,fill=class))+
  ...
  annotate("text",x=17.2,y=4.42,label="1 tile = 1 FDR-supported model",hjust=1,family=font_family,size=1.45,colour=pal[["grey"]])+
...
pD<-ggplot()+
  ...
  annotate("text",x=.315,y=.45,label="Transparent reporting  ·  MAGMA priority  ·  kidney cortex proxy context  ·  interpretive exemplars",family=font_family,fontface="bold",size=1.60)+
  annotate("text",x=.8125,y=.45,label="Causal genes  ·  validated genes\nSMR/coloc support  ·  therapeutic targets",family=font_family,fontface="bold",size=1.55,lineheight=.9)+
...
width_in<-7.205;height_in<-5.45;stem<-file.path(fig_dir,"figure3_candidate_evidence_model_draft_v0.6R")
...
preview_path<-file.path(log_dir,"figure3_candidate_evidence_model_v0.6R_qc_50pct.png")
...
log_lines<-c(paste0("timestamp=",format(Sys.time(),"%Y-%m-%dT%H:%M:%S%z")),"backend=R ggplot2/patchwork/svglite/pdf/ragg","figure=Figure 3 v0.6R",
  ...
  "baseline=Figure 3 v0.6 true evidence flow","structure_changed_from_v0.6=no","source_data_changed_from_v0.6=no","claim_changed_from_v0.6=no",paste0("canvas_inches=",width_in,"x",height_in),"font=Helvetica","png_dpi=600","figure2_modified=no","figure4_started=no")
writeLines(log_lines,file.path(log_dir,"stage7E_generate_figure3_v0.6R.log"));cat(paste(log_lines,collapse="\n"),"\n")
