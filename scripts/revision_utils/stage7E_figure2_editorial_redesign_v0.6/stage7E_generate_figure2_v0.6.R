...
fig_dir <- file.path(root, "results/figures/revision/stage7E_figure2_editorial_redesign_v0.6")
table_dir <- file.path(root, "results/tables/revision/stage7E_figure2_editorial_redesign_v0.6")
...
fwrite(display_mapping, file.path(table_dir, "figure2_display_label_mapping_v0.6.tsv"), sep = "\t")
...
pA <- ggplot() +
  annotate("segment", x = c(0.07, 0.10, 0.13, 0.16, 0.19),
           ...
           color = pal[["deep_teal"]], linewidth = 1.05, lineend = "round") +
  annotate("text", x = 0.23, y = 0.885, label = "MAGMA-prioritized\nmodules", hjust = 0,
           family = font_family, fontface = "bold", size = 1.55, lineheight = 0.88, color = pal[["deep_teal"]]) +
  annotate("segment", x = 0.56, xend = 0.56, y = 0.82, yend = 0.765,
           color = pal[["mid_grey"]], linewidth = 0.38,
           arrow = grid::arrow(length = grid::unit(2.4, "pt"), type = "closed")) +
  annotate("text", x = 0.56, y = 0.72, label = "GSE231569 papilla snRNA",
           family = font_family, fontface = "bold", size = 1.95, color = pal[["deep_teal"]]) +
...
pB <- ggplot(dB, aes(x = donor_display, y = compartment_display, fill = mean_module_score)) +
  ...
  theme_pub(7.5) +
  theme(
    axis.text.x = element_text(size = 6.7), axis.text.y = element_text(size = 6.8),
...
pC <- ggplot() +
  ...
  annotate("point", x = c(0.10, 0.15, 0.20, 0.25), y = 0.80, shape = 21, size = 2.6,
           fill = pal[["pale_teal"]], color = pal[["loop_teal"]], stroke = 0.45) +
  annotate("point", x = c(0.10, 0.15, 0.20, 0.25), y = 0.50, shape = 22, size = 2.6,
           fill = pal[["muted_teal"]], color = pal[["deep_teal"]], stroke = 0.45) +
  annotate("point", x = c(0.08, 0.125, 0.17, 0.215, 0.26), y = 0.20, shape = 22, size = 2.35,
           fill = pal[["muted_teal"]], color = pal[["deep_teal"]], stroke = 0.42) +
  ...
  annotate("text", x = 0.32, y = 0.155, label = "Retained after 5/5 removal panels", hjust = 0,
           family = font_family, size = 1.75, color = pal[["grey"]]) +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1), clip = "off") +
  labs(title = "Consistency and robustness\nsummary", caption = "Detailed matrices in Source Data") +
  theme_void(base_family = font_family) +
  theme(
    plot.title = element_text(family = font_family, face = "bold", size = 8.1, lineheight = 0.92,
                              color = pal[["dark"]], margin = margin(b = 2, l = 13)),
    plot.caption = element_text(family = font_family, size = 5.7, color = pal[["grey"]], hjust = 0, margin = margin(t = 2)),
...
pD <- ggplot(dD, aes(y = module_display)) +
  ...
  annotate("label", x = 119, y = 4.80,
           label = "Delta above matched random\nRank metrics saturated\nOverall: partial support",
           hjust = 1, family = font_family, fontface = "bold", size = 1.85,
           lineheight = 0.92, fill = pal[["pale_amber"]], color = pal[["dark_amber"]],
           linewidth = 0.28, label.padding = grid::unit(1.7, "pt")) +
  scale_x_continuous(breaks = c(0, 50, 100), limits = c(0, 122), expand = c(0, 0)) +
  scale_y_discrete(expand = expansion(add = c(0.25, 1.55))) +
...
e_xmin <- c(0.02, 0.215, 0.41, 0.655, 0.825)
e_xmax <- c(0.205, 0.40, 0.595, 0.815, 0.985)
pE <- ggplot() +
...
for (i in seq_along(e_labels)) {
  pE <- pE +
    ...
    annotate("text", x = (e_xmin[i] + e_xmax[i]) / 2, y = 0.455, label = e_labels[i],
             family = font_family, fontface = "bold", size = 2.0, color = pal[["dark"]], lineheight = 0.88)
}
pE <- pE +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1), clip = "off") +
...
width_in <- 7.205
height_in <- 6.45
stem <- file.path(fig_dir, "figure2_snRNA_context_draft_v0.6")
...
if (requireNamespace("magick", quietly = TRUE)) {
  ...
  magick::image_write(half_preview, file.path(log_dir, "figure2_snRNA_context_v0.6_qc_50pct.png"), format = "png")
  ...
  png::writePNG(half_preview, file.path(log_dir, "figure2_snRNA_context_v0.6_qc_50pct.png"))
}

log_lines <- c(
  paste0("timestamp=", format(Sys.time(), "%Y-%m-%dT%H:%M:%S%z")),
  "backend=R ggplot2/patchwork/svglite/pdf/ragg",
  "figure=Figure 2 v0.6",
...
writeLines(log_lines, file.path(log_dir, "stage7E_generate_figure2_v0.6.log"))
