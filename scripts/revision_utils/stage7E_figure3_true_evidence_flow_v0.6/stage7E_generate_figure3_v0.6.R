...
fig_dir <- file.path(root, "results/figures/revision/stage7E_figure3_true_evidence_flow_v0.6")
table_dir <- file.path(root, "results/tables/revision/stage7E_figure3_true_evidence_flow_v0.6")
log_dir <- file.path(root, "logs/revision/stage7E_figure3_true_evidence_flow_v0.6")
...
pA<-ggplot()+
  ...
  annotate("text",x=.48,y=.36,label="MAGMA Bonferroni\n93",family=font_family,fontface="bold",size=1.8,lineheight=.88)+
  annotate("text",x=.48,y=.767,label="Lower MAGMA\n33",family=font_family,fontface="bold",size=1.65,lineheight=.88)+
  annotate("text",x=.48,y=.955,label="TWAS proxy only  4",family=font_family,fontface="bold",size=1.5,colour=pal[["gold_dark"]])+
  ...
  annotate("segment",x=.90,xend=.935,y=.563,yend=.563,colour=pal[["gold"]],linewidth=.35)+
  annotate("text",x=.94,y=.563,label="R2  multi-SNP  1",hjust=0,family=font_family,fontface="bold",size=1.4,colour=pal[["gold_dark"]])+
  annotate("segment",x=.90,xend=.935,y=.886,yend=.886,colour=pal[["gold"]],linewidth=.35)+
  annotate("text",x=.94,y=.886,label="R5  proxy only  4",hjust=0,family=font_family,fontface="bold",size=1.4,colour=pal[["gold_dark"]])+
  annotate("segment",x=.90,xend=.90,y=.935,yend=.975,colour=pal[["mid"]],linewidth=.6)+
  annotate("text",x=.92,y=.975,label="R6  contextual / unresolved  0",hjust=0,family=font_family,fontface="bold",size=1.35,colour=pal[["grey"]])+
  annotate("text",x=.13,y=.995,label="CANDIDATES",family=font_family,fontface="bold",size=1.55,colour=pal[["grey"]])+
  annotate("text",x=.48,y=.995,label="EVIDENCE AXIS",family=font_family,fontface="bold",size=1.55,colour=pal[["grey"]])+
  annotate("text",x=.85,y=.995,label="REPORTING GROUP",family=font_family,fontface="bold",size=1.55,colour=pal[["grey"]])+
...
for(i in seq_along(exemplar_names)) pC<-pC+annotate("rect",xmin=chip_x[i]-.105,xmax=chip_x[i]+.105,ymin=chip_y[i]-.07,ymax=chip_y[i]+.07,fill="#FBFAF6",colour="#D2C39E",linewidth=.25)+
  annotate("text",x=chip_x[i],y=chip_y[i],label=exemplar_names[i],family=font_family,fontface="bold.italic",size=1.75)
...
width_in<-7.205;height_in<-5.45;stem<-file.path(fig_dir,"figure3_candidate_evidence_model_draft_v0.6")
...
preview_path<-file.path(log_dir,"figure3_candidate_evidence_model_v0.6_qc_50pct.png")
...
log_lines<-c(paste0("timestamp=",format(Sys.time(),"%Y-%m-%dT%H:%M:%S%z")),"backend=R ggplot2/patchwork/svglite/pdf/ragg","figure=Figure 3 v0.6",
  ...
  "source_data_changed_from_v0.5=no","claim_changed_from_v0.5=no",paste0("canvas_inches=",width_in,"x",height_in),"font=Helvetica","png_dpi=600","figure2_modified=no","figure4_started=no")
writeLines(log_lines,file.path(log_dir,"stage7E_generate_figure3_v0.6.log"));cat(paste(log_lines,collapse="\n"),"\n")
