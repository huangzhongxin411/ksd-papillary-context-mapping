...
stage_id <- "stage7E_figure4_final_polish_v0.4"
...
log_path <- file.path(log_dir, "stage7E_figure4_final_polish_v0.4.log")
...
pB <- ggplot(panelB, aes(x = module_short, y = paired_delta)) +
  ...
  geom_violin(width = 0.78, fill = "#EDF3F4", color = NA, trim = TRUE, alpha = 0.72) +
  geom_boxplot(width = 0.28, outlier.shape = NA, fill = "white", color = teal,
               linewidth = 0.55) +
  geom_jitter(width = 0.095, height = 0, size = 1.15, alpha = 0.58, color = teal) +
...
seg[, `:=`(xmin = stage_x - 0.39 + 0.78 * cum0,
           ...
           ymin = module_y - 0.16, ymax = module_y + 0.16,
           xmid = (stage_x - 0.39 + 0.78 * cum0 + stage_x - 0.39 + 0.78 * (cum0 + fraction)) / 2)]
pE <- ggplot() +
  geom_tile(data = tile_base, aes(x = stage_x, y = module_y), width = 0.86, height = 0.58,
            fill = "#FAFBFB", color = line_col, linewidth = 0.5) +
  ...
  annotate("text", x = 1.5, y = 4.72,
           label = "Paired bulk shift positive in all four modules",
           family = font_family, fontface = "bold", size = 3.25, color = teal) +
  scale_fill_manual(values = c("Direction retained" = teal_mid, "Attenuated" = gold,
                               "Lost / explained" = brick_mid), drop = FALSE) +
  scale_x_continuous(breaks = 1:2, labels = stage_labels, position = "top",
                     expand = expansion(add = 0.46)) +
  scale_y_continuous(breaks = 4:1, labels = unname(short_labels),
                     expand = expansion(add = c(0.42, 0.80))) +
...
pdf_path <- file.path(fig_dir, "figure4_gse73680_bulk_context_draft_v0.4.pdf")
png_path <- file.path(fig_dir, "figure4_gse73680_bulk_context_draft_v0.4.png")
svg_path <- file.path(fig_dir, "figure4_gse73680_bulk_context_draft_v0.4.svg")
...
fwrite(manifest, file.path(table_dir, "figure4_source_data_manifest_v0.4.tsv"), sep = "\t")

changes <- data.table(
  panel = c("overall", "A", "B", "C", "D", "E", "F", "overall", "overall"),
  issue_in_v0.3 = c("Final hierarchy needed polish", "Resource strip remained visually prominent", "Points and violin fill could be lighter", "CI and FDR note needed refinement", "Heatmap needed final spacing polish", "Three-column matrix retained a table-like appearance", "Boundary strip needed alignment polish", "Typography needed final consistency", "Palette needed final tonal balance"),
  change_made_in_v0.4 = c("Refined spacing and panel balance", "Lightened strip and centered arrow annotation", "Reduced points and lightened violin", "Strengthened CI/points and consolidated approximate FDR note", "Retained rho-only heatmap with simplified presentation", "Removed repeated positive column and used two segmented adjustment bars", "Preserved six equal boundary boxes with refined spacing", "Maintained Helvetica and reduced internal text", "Softened background fills while preserving semantic colors"),
  data_changed = "no",
  claim_changed = "no",
...
fwrite(changes, file.path(table_dir, "figure4_visual_change_log_v0.4.tsv"), sep = "\t")
