...
stage_id <- "stage7E_figure5_final_micro_polish_v0.6"
...
log_path <- file.path(log_dir, "stage7E_figure5_final_micro_polish_v0.6.log")
...
figure5 <- (
  ...
  plot_layout(heights = c(0.76, 1.40, 0.66, 0.47), widths = c(0.96, 1.04)) +
  plot_annotation(
...
pdf_path <- file.path(fig_dir, "figure5_spatial_twas_context_draft_v0.6.pdf")
png_path <- file.path(fig_dir, "figure5_spatial_twas_context_draft_v0.6.png")
svg_path <- file.path(fig_dir, "figure5_spatial_twas_context_draft_v0.6.svg")
...
fwrite(manifest, file.path(table_dir, "figure5_source_data_manifest_v0.6.tsv"), sep = "\t")

changes <- data.table(
  panel = c("overall", LETTERS[1:6]),
  issue_in_v0.5 = c(
    "Accepted figure required final micro-polish",
    "Display-section note was split and a duplicate label floated under B",
    "Projection images retained avoidable internal whitespace",
    "Cell labels and status key needed one final readability step",
    "Median trajectory needed slightly stronger contrast",
    "Boundary pills needed more internal padding",
    "Boundary zones needed slightly greater visual weight"
  ),
  polish_made_in_v0.6 = c(
    "Preserved evidence-weighted geometry and harmonized final spacing",
    "Integrated the complete GSM6250309 note into Panel A and removed the orphan caption",
    "Expanded the top band and reduced margins while preserving secondary status",
    "Increased cell and status-key typography slightly",
    "Strengthened median line, lightened pair lines, and balanced facet titles",
    "Increased pill padding while retaining equal-height cards",
    "Increased zone height, padding, and text readability"
  ),
  data_changed = "no",
  claim_changed = "no",
...
fwrite(changes, file.path(table_dir, "figure5_visual_polish_log_v0.6.tsv"), sep = "\t")
