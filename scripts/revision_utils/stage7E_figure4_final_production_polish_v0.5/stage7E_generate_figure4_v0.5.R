...
stage_id <- "stage7E_figure4_final_production_polish_v0.5"
...
log_path <- file.path(log_dir, "stage7E_figure4_final_production_polish_v0.5.log")
...
pD <- ggplot(panelF, aes(x = signature_short, y = module_short, fill = spearman_r)) +
  ...
  scale_fill_gradient(low = "#F7F3F2", high = "#B47A6B", limits = c(0, 0.85),
...
pE <- ggplot() +
  geom_tile(data = tile_base, aes(x = stage_x, y = module_y), width = 0.82, height = 0.42,
  ...
  annotate("text", x = 1.5, y = 4.78,
           label = "Paired bulk shift positive in all four modules",
           family = font_family, fontface = "bold", size = 3.10, color = teal) +
  annotate("text", x = 1, y = 4.22, label = "Single-signature\nadjustment",
           family = font_family, fontface = "bold", size = 3.15, lineheight = 0.92, color = ink) +
  annotate("text", x = 2, y = 4.22, label = "Compact\nadjustment",
           family = font_family, fontface = "bold", size = 3.15, lineheight = 0.92, color = ink) +
...
pdf_path <- file.path(fig_dir, "figure4_gse73680_bulk_context_draft_v0.5.pdf")
png_path <- file.path(fig_dir, "figure4_gse73680_bulk_context_draft_v0.5.png")
svg_path <- file.path(fig_dir, "figure4_gse73680_bulk_context_draft_v0.5.svg")
...
fwrite(manifest, file.path(table_dir, "figure4_source_data_manifest_v0.5.tsv"), sep = "\t")

changes <- data.table(
  panel = c("overall", "A", "B", "C", "D", "E", "F", "overall", "overall"),
  issue_in_v0.4 = c("Whole-figure production balance needed final pass", "Resource strip was slightly faint", "Minor overplotting remained", "CI/FDR placement required final confirmation", "Heatmap saturation remained visually strong", "Shared note and column headers competed vertically; mini-bars retained cell outlines", "Boundary strip required final production consistency", "Typography needed final production check", "Color restraint required final harmonization"),
  polish_made_in_v0.5 = c("Harmonized final spacing and tonal hierarchy", "Raised strip contrast while retaining subtle emphasis", "Slightly reduced points and lightened violins", "Retained stronger CIs and clean approximate FDR note", "Reduced terracotta saturation and centered colorbar", "Placed shared note directly below subtitle, moved headers near bars, removed heavy cell borders", "Preserved equal-height six-box strip and semantic fills", "Maintained Helvetica hierarchy and readable minimum sizes", "Softened Panel D while preserving state colors in Panel E/F"),
  data_changed = "no",
  claim_changed = "no",
...
fwrite(changes, file.path(table_dir, "figure4_visual_polish_log_v0.5.tsv"), sep = "\t")
