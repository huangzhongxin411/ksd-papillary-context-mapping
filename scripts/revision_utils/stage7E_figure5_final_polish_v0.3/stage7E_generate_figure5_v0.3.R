...
stage_id <- "stage7E_figure5_final_polish_v0.3"
...
log_path <- file.path(log_dir, "stage7E_figure5_final_polish_v0.3.log")
...
pA <- ggplot(panelA, aes(y = section_label, x = n_tissue_spots, fill = representative_for_display)) +
  ...
  annotate("rect", xmin = 1690, xmax = 2980, ymin = 5.52, ymax = 5.92,
           fill = brick_light, color = NA) +
  annotate("text", x = 2335, y = 5.72, label = "NO PLAQUE / MINERAL / LESION ROI",
...
pB <- ggplot(panelB, aes(x = x_hires, y = y_hires_plot)) +
  ...
  scale_color_gradient2(low = teal_mid, mid = "#F1F3F3", high = "#B9877A", midpoint = 0,
...
pC <- ggplot(panelC, aes(x = context_display, y = module_display,
  ...
  geom_text(aes(label = cell_label), family = font_family, size = 3.05,
...
pD <- ggplot(panelD_long, aes(x = score_type, y = spearman_r, group = pair_id)) +
  ...
  geom_line(color = "#AEB7BA", alpha = 0.25, linewidth = 0.32) +
  geom_point(color = teal_mid, size = 1.15, alpha = 0.47) +
  geom_line(data = panelD_median, aes(x = score_type, y = median_rho, group = 1),
            inherit.aes = FALSE, color = ink, linewidth = 1.12) +
...
pE <- ggplot() +
  annotate("rect", xmin = 0.03, xmax = 0.43, ymin = 0.12, ymax = 0.88,
           fill = "#F5F7F7", color = line_col, linewidth = 0.5) +
  annotate("rect", xmin = 0.47, xmax = 0.97, ymin = 0.12, ymax = 0.88,
           fill = "#FAF8F6", color = line_col, linewidth = 0.5) +
  annotate("text", x = 0.06, y = 0.79, label = "TWAS PROXY BURDEN", hjust = 0,
           family = font_family, fontface = "bold", size = 3.2, color = teal) +
  annotate("text", x = 0.06, y = 0.62, label = "51", hjust = 0,
           family = font_family, fontface = "bold", size = 7.0, color = ink) +
  annotate("text", x = 0.145, y = 0.62, label = "FDR-supported genes", hjust = 0,
  ...
  annotate("text", x = 0.50, y = 0.79, label = "BOUNDARY NOTES", hjust = 0,
           family = font_family, fontface = "bold", size = 3.2, color = brick) +
  annotate("text", x = 0.50, y = 0.42,
           label = "GTEx kidney cortex proxy only\nR2: one-gene descriptive\nR5: not score-feasible\nNo papilla-specific eQTL\nNo SMR/coloc support",
           hjust = 0, vjust = 0.5, family = font_family, size = 2.9,
           lineheight = 1.32, color = ink) +
...
# F. Seven-statement interpretation boundary.
boundary <- data.table(
  x = 1:7,
  group = c(rep("SUPPORTED INTERPRETATION", 3), rep("NOT CLAIMED", 4)),
  label = c(
...
pF <- ggplot(boundary, aes(x = x, y = 1, fill = group)) +
  geom_tile(width = 0.94, height = 0.65, color = "white", linewidth = 0.6) +
  geom_text(aes(label = label), family = font_family, fontface = "bold",
            size = 2.85, lineheight = 0.92, color = ink) +
  annotate("text", x = 2, y = 1.40, label = "SUPPORTED INTERPRETATION",
           family = font_family, fontface = "bold", size = 3.0, color = teal) +
  annotate("text", x = 5.5, y = 1.40, label = "NOT CLAIMED",
  ...
  coord_cartesian(ylim = c(0.60, 1.55), clip = "off") +
...
figure5 <- (
  ...
  plot_layout(heights = c(0.90, 1.12, 0.62, 0.43), widths = c(0.84, 1.16)) +
  plot_annotation(
...
pdf_path <- file.path(fig_dir, "figure5_spatial_twas_context_draft_v0.3.pdf")
png_path <- file.path(fig_dir, "figure5_spatial_twas_context_draft_v0.3.png")
svg_path <- file.path(fig_dir, "figure5_spatial_twas_context_draft_v0.3.svg")
...
fwrite(manifest, file.path(table_dir, "figure5_source_data_manifest_v0.3.tsv"), sep = "\t")

changes <- data.table(
  panel = c("overall", LETTERS[1:6]),
  issue_in_v0.2 = c(
    "Final visual harmonization remained",
    "No-ROI element was wider than necessary",
    "Representative overlay required final restraint check",
    "Status abbreviations lacked a compact on-panel key",
    "Thin pair trajectories remained slightly dense",
    "Right boundary card needed more line spacing",
    "Boundary strip required final border harmonization"
  ),
  polish_made_in_v0.3 = c(
    "Confirmed six-panel hierarchy and Figure 1-4 visual family",
    "Converted no-ROI element into a compact restrained badge",
    "Retained subdued overlay and visualization-only boundary",
    "Added compact CP/ATT/NS/MIX legend and refined cell readability",
    "Lowered pair-line alpha and strengthened median line",
    "Equalized cards and increased right-card line spacing",
    "Reduced border weight and confirmed equal-height grouped boxes"
  ),
  data_changed = "no",
  claim_changed = "no",
...
fwrite(changes, file.path(table_dir, "figure5_visual_polish_log_v0.3.tsv"), sep = "\t")
