...
fig_dir <- file.path(root, "results/figures/revision/stage7E_figure2_final_polish_v0.5")
table_dir <- file.path(root, "results/tables/revision/stage7E_figure2_final_polish_v0.5")
...
fwrite(display_mapping, file.path(table_dir, "figure2_display_label_mapping_v0.5.tsv"), sep = "\t")
...
nephron_base <- data.table(
  x = c(0.14, 0.10, 0.09, 0.11, 0.15, 0.19, 0.21),
  y = c(0.69, 0.59, 0.43, 0.30, 0.23, 0.35, 0.65)
)
nephron_tal <- data.table(
  x = c(0.15, 0.18, 0.20, 0.21),
  y = c(0.23, 0.34, 0.48, 0.65)
)
pA <- ggplot() +
  annotate("text", x = 0.52, y = 0.88, label = "GSE231569 papilla snRNA",
           family = font_family, fontface = "bold", size = 2.4, color = pal[["deep_teal"]]) +
  ...
  annotate("point", x = 0.14, y = 0.69, shape = 21, size = 3.6,
           fill = pal[["white"]], color = pal[["muted_teal"]], stroke = 0.7) +
  annotate("segment", x = 0.145, xend = 0.205, y = 0.745, yend = 0.64,
           color = pal[["loop_teal"]], linewidth = 0.4) +
  annotate("text", x = 0.145, y = 0.78, label = "Loop/TAL", hjust = 0.5,
           family = font_family, fontface = "bold", size = 1.55, color = pal[["loop_teal"]]) +
  annotate("rect", xmin = 0.35, xmax = 0.64, ymin = 0.52, ymax = 0.72,
           fill = pal[["very_light"]], color = pal[["light_grey"]], linewidth = 0.35) +
  annotate("rect", xmin = 0.67, xmax = 0.96, ymin = 0.52, ymax = 0.72,
           fill = pal[["pale_teal"]], color = pal[["loop_teal"]], linewidth = 0.42) +
  annotate("rect", xmin = 0.35, xmax = 0.64, ymin = 0.29, ymax = 0.49,
           fill = pal[["very_light"]], color = pal[["light_grey"]], linewidth = 0.35) +
  annotate("rect", xmin = 0.67, xmax = 0.96, ymin = 0.29, ymax = 0.49,
           fill = pal[["very_light"]], color = pal[["light_grey"]], linewidth = 0.35) +
  annotate("text", x = 0.495, y = 0.63, label = "43,878", family = font_family,
           fontface = "bold", size = 3.05, color = pal[["deep_teal"]]) +
  annotate("text", x = 0.495, y = 0.56, label = "nuclei", family = font_family,
           size = 1.65, color = pal[["grey"]]) +
  annotate("text", x = 0.815, y = 0.63, label = "540", family = font_family,
           fontface = "bold", size = 3.05, color = pal[["loop_teal"]]) +
  annotate("text", x = 0.815, y = 0.56, label = "Loop/TAL", family = font_family,
           size = 1.65, color = pal[["grey"]]) +
  annotate("text", x = 0.495, y = 0.40, label = "4", family = font_family,
           fontface = "bold", size = 3.05, color = pal[["deep_teal"]]) +
  annotate("text", x = 0.495, y = 0.33, label = "donors", family = font_family,
           size = 1.65, color = pal[["grey"]]) +
  annotate("text", x = 0.815, y = 0.40, label = "6", family = font_family,
           fontface = "bold", size = 3.05, color = pal[["deep_teal"]]) +
  annotate("text", x = 0.815, y = 0.33, label = "compartments", family = font_family,
           size = 1.55, color = pal[["grey"]]) +
  annotate("rect", xmin = 0.35, xmax = 0.96, ymin = 0.11, ymax = 0.20,
           fill = pal[["very_light"]], color = NA) +
  annotate("text", x = 0.655, y = 0.155, label = "Donor \u00d7 compartment summaries",
           family = font_family, fontface = "bold", size = 1.5, color = pal[["grey"]]) +
...
pB <- ggplot(dB, aes(x = donor_display, y = compartment_display, fill = mean_module_score)) +
  geom_tile(color = pal[["white"]], linewidth = 0.24) +
  ...
  scale_fill_gradientn(
    ...
    name = "Mean score",
    guide = guide_colorbar(barheight = grid::unit(18, "mm"), barwidth = grid::unit(2.8, "mm"),
...
pC <- ggplot(dC, aes(x = donor_display, y = module_display)) +
  geom_tile(fill = pal[["pale_teal"]], color = pal[["white"]], linewidth = 0.45) +
  geom_text(aes(label = loop_tal_rank), family = font_family, fontface = "bold",
            size = 2.45, color = pal[["deep_teal"]]) +
  labs(title = "Loop/TAL rank",
       subtitle = "Rank 1 = highest; descriptive only\nTop-ranked in 4/4 donors",
       x = "Donor", y = NULL) +
  theme_pub(7.0) +
...
pD <- ggplot(dD, aes(x = donor_display, y = module_display)) +
  geom_tile(fill = pal[["very_light"]], color = pal[["white"]], linewidth = 0.45) +
...
pE <- ggplot(dE, aes(y = module_display)) +
  ...
  annotate("label", x = 126, y = 4.58, label = "Overall: partial support",
           hjust = 1, family = font_family, fontface = "bold", size = 1.75,
           fill = pal[["pale_amber"]], color = pal[["dark_amber"]],
           linewidth = 0.25, label.padding = grid::unit(1.2, "pt")) +
  scale_x_continuous(breaks = c(0, 50, 100), limits = c(0, 128), expand = c(0, 0)) +
  scale_y_discrete(expand = expansion(add = c(0.25, 0.85))) +
  labs(title = "Matched-random benchmark", subtitle = "Partial matched-random support",
       x = "Matched-random percentile", y = NULL,
       caption = "Delta above matched random; rank metrics saturated") +
  theme_pub(7.0) +
...
pF <- ggplot(dF, aes(x = removal_display, y = module_display)) +
  ...
  geom_point(data = dF[retained == TRUE], shape = 22, size = 4.0, stroke = 0.45,
             fill = pal[["muted_teal"]], color = pal[["deep_teal"]]) +
  labs(title = "Driver-panel removal", subtitle = "Descriptive robustness\nFilled = retained", x = NULL, y = NULL) +
  theme_pub(7.1) +
  theme(axis.line = element_blank(), axis.ticks = element_blank(), panel.grid = element_blank(),
...
for (i in seq_along(g_labels)) {
  pG <- pG +
    annotate("rect", xmin = g_xmin[i], xmax = g_xmax[i], ymin = 0.18, ymax = 0.75,
             fill = g_fill[i], color = g_border[i], linewidth = 0.42) +
    annotate("text", x = (g_xmin[i] + g_xmax[i]) / 2, y = 0.465, label = g_labels[i],
             family = font_family, fontface = "bold", size = 1.76, color = pal[["dark"]], lineheight = 0.86)
}
pG <- pG +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1), clip = "off") +
...
width_in <- 7.205
height_in <- 7.05
stem <- file.path(fig_dir, "figure2_snRNA_context_draft_v0.5")
...
if (requireNamespace("magick", quietly = TRUE)) {
  ...
  magick::image_write(half_preview, file.path(log_dir, "figure2_snRNA_context_v0.5_qc_50pct.png"), format = "png")
  ...
  png::writePNG(half_preview, file.path(log_dir, "figure2_snRNA_context_v0.5_qc_50pct.png"))
}

log_lines <- c(
  paste0("timestamp=", format(Sys.time(), "%Y-%m-%dT%H:%M:%S%z")),
  "backend=R ggplot2/patchwork/svglite/pdf/ragg",
  "figure=Figure 2 v0.5",
...
writeLines(log_lines, file.path(log_dir, "stage7E_generate_figure2_v0.5.log"))
