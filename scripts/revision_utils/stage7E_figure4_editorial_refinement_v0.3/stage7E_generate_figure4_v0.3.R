...
stage_id <- "stage7E_figure4_editorial_refinement_v0.3"
...
log_path <- file.path(log_dir, "stage7E_figure4_editorial_refinement_v0.3.log")
...
pC <- ggplot(panelC, aes(y = module_short, x = effect_estimate)) +
  ...
  annotate("text", x = max(panelC$ci_high) + 0.012, y = 4.15,
           label = "All FDR = 0.036", hjust = 0, family = font_family,
           size = 3.0, color = muted) +
...
pD <- ggplot(panelF, aes(x = signature_short, y = module_short, fill = spearman_r)) +
  ...
  scale_fill_gradient(low = "#F5F1F0", high = brick, limits = c(0, 0.85),
  ...
  theme(axis.text.x = element_text(angle = 21, hjust = 1),
        ...
        legend.key.width = unit(1.15, "cm"), legend.key.height = unit(0.28, "cm"),
        plot.margin = margin(6, 7, 5, 7))

# E. Tile-based before/after summary with source-backed outcome proportions.
stage_labels <- c("Paired bulk shift", "Single-signature\nadjustment", "Compact\nadjustment")
tile_base <- CJ(module_short = factor(unname(short_labels), levels = unname(short_labels)), stage_x = 1:3)
tile_base[, module_y := 5 - as.numeric(module_short)]
paired_seg <- panelC[, .(module_short = factor(as.character(module_short), levels = unname(short_labels)),
                         stage_x = 1L, outcome = factor("Direction retained",
                         levels = c("Direction retained", "Attenuated", "Lost / explained")),
                         fraction = 1, n_models = 1L)]
adjust_seg <- copy(panelE)
adjust_seg[, stage_x := fifelse(model_family == "Single-signature", 2L, 3L)]
adjust_seg <- adjust_seg[, .(module_short, stage_x, outcome, fraction, n_models)]
seg <- rbindlist(list(paired_seg, adjust_seg), use.names = TRUE)
seg[, module_y := 5 - as.numeric(module_short)]
setorder(seg, module_y, stage_x, outcome)
seg[, cum0 := shift(cumsum(fraction), fill = 0), by = .(module_short, stage_x)]
seg[, `:=`(xmin = stage_x - 0.40 + 0.80 * cum0,
           xmax = stage_x - 0.40 + 0.80 * (cum0 + fraction),
           ymin = module_y - 0.24, ymax = module_y - 0.03)]
tile_labels <- tile_base[, .(module_short, module_y, stage_x)]
tile_labels[, label := fifelse(stage_x == 1, "Positive",
                        fifelse(stage_x == 2, "3 retained · 3 attenuated · 2 lost",
                                             "3 retained · 2 lost"))]
pE <- ggplot() +
  geom_tile(data = tile_base, aes(x = stage_x, y = module_y), width = 0.88, height = 0.72,
            fill = "#F7F8F8", color = line_col, linewidth = 0.55) +
  geom_rect(data = seg, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, fill = outcome),
            color = "white", linewidth = 0.35) +
  geom_text(data = tile_labels, aes(x = stage_x, y = module_y + 0.13, label = label),
            family = font_family, size = 3.0, color = ink) +
  scale_fill_manual(values = c("Direction retained" = teal_mid, "Attenuated" = gold,
                               "Lost / explained" = brick_mid), drop = FALSE) +
  scale_x_continuous(breaks = 1:3, labels = stage_labels, position = "top",
                     expand = expansion(add = 0.48)) +
  scale_y_continuous(breaks = 4:1, labels = unname(short_labels),
...
pdf_path <- file.path(fig_dir, "figure4_gse73680_bulk_context_draft_v0.3.pdf")
png_path <- file.path(fig_dir, "figure4_gse73680_bulk_context_draft_v0.3.png")
svg_path <- file.path(fig_dir, "figure4_gse73680_bulk_context_draft_v0.3.svg")
...
fwrite(manifest, file.path(table_dir, "figure4_source_data_manifest_v0.3.tsv"), sep = "\t")

changes <- data.table(
  panel = c("overall", "A", "B", "C", "D", "E", "F", "overall", "overall"),
  issue_in_v0.2 = c("Evidence flow remained visually dense", "Info strip omitted paired count", "Full module names crowded the axis", "Repeated FDR labels added clutter", "Cell-level FDR text and mini bars overfilled the panel", "Stacked bars obscured the before/after attenuation message", "Boundary strip required minor consistency refinement", "Typography needed tighter hierarchy", "Palette needed final semantic consistency"),
  change_made_in_v0.3 = c("Reduced internal text and strengthened attenuation sequence", "Consolidated all cohort metrics into one strip", "Applied R1/R1-R3/Top 50/Top 100 labels", "Replaced repeated labels with one concise FDR note", "Removed mini bars and retained rho-only heatmap", "Rebuilt as three-column tile summary with source-backed outcome strips", "Retained full-width supported/not-claimed structure", "Maintained Helvetica with simplified labels", "Retained restrained teal/gold/terracotta"),
  data_changed = "no",
  claim_changed = "no",
...
fwrite(changes, file.path(table_dir, "figure4_visual_change_log_v0.3.tsv"), sep = "\t")
