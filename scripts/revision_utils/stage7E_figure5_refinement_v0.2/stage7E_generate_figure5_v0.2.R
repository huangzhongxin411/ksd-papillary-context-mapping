...
stage_id <- "stage7E_figure5_refinement_v0.2"
...
log_path <- file.path(log_dir, "stage7E_figure5_refinement_v0.2.log")
...
pA <- ggplot(panelA, aes(y = section_label, x = n_tissue_spots, fill = representative_for_display)) +
  ...
  annotate("rect", xmin = 1200, xmax = 2980, ymin = 5.48, ymax = 5.96,
           fill = brick_light, color = NA) +
  annotate("text", x = 2090, y = 5.72, label = "NO PLAQUE / MINERAL / LESION ROI",
...
pC <- ggplot(panelC, aes(x = context_display, y = module_display,
  ...
  labs(title = "Adjusted section-level co-distribution",
       subtitle = "Five-section median after complexity adjustment", x = NULL, y = NULL) +
  theme_pub(8.8) +
...
pD <- ggplot(panelD_long, aes(x = score_type, y = spearman_r, group = pair_id)) +
  ...
  geom_line(color = "#AEB7BA", alpha = 0.32, linewidth = 0.34) +
  geom_point(color = teal_mid, size = 1.2, alpha = 0.52) +
  geom_line(data = panelD_median, aes(x = score_type, y = median_rho, group = 1),
            inherit.aes = FALSE, color = ink, linewidth = 1.05) +
  ...
  labs(title = "Complexity-adjustment changes",
       ...
       x = NULL, y = "Within-section Spearman rho") +
  theme_pub(8.2) +
  theme(axis.text.x = element_text(angle = 20, hjust = 1),
...
pF <- ggplot(boundary, aes(x = x, y = 1, fill = group)) +
  geom_tile(width = 0.94, height = 0.65, color = "white", linewidth = 0.7) +
...
pdf_path <- file.path(fig_dir, "figure5_spatial_twas_context_draft_v0.2.pdf")
png_path <- file.path(fig_dir, "figure5_spatial_twas_context_draft_v0.2.png")
svg_path <- file.path(fig_dir, "figure5_spatial_twas_context_draft_v0.2.svg")
...
fwrite(manifest, file.path(table_dir, "figure5_source_data_manifest_v0.2.tsv"), sep = "\t")

changes <- data.table(
  panel = c("overall", LETTERS[1:6]),
  issue_in_v0.1 = c(
    "Dense draft hierarchy",
    "Resource panel read mainly as a bar chart",
    "Representative overlay could appear evidentiary",
    "Adjusted evidence matrix needed greater emphasis",
    "Raw-to-adjusted changes were visually dense",
    "TWAS boundary was text-heavy",
    "Boundary strip mixed support and exclusions"
  ),
  change_made_in_v0.2 = c(
    "Rebalanced six-panel hierarchy around C/D and conservative boundaries",
    "Added explicit no-ROI badge and QC-selection note",
    "Reduced visual weight and added visualization-only subtitle",
    "Enlarged heatmap with concise rho/status cells",
    "Reduced pair-line alpha and emphasized medians",
    "Rebuilt as two-part proxy-burden/boundary card",
    "Rebuilt as grouped supported/not-claimed strip"
  ),
  data_changed = "no",
  claim_changed = "no",
...
fwrite(changes, file.path(table_dir, "figure5_visual_change_log_v0.2.tsv"), sep = "\t")
