# Template: ranked lollipop.
# Required columns: label, score, group. Optional: size_value.

make_lollipop_template <- function(dt, title, xlab = "Score") {
  if (!"size_value" %in% names(dt)) dt$size_value <- 1
  dt$label <- factor(dt$label, levels = rev(dt$label))
  ggplot(dt, aes(score, label)) +
    geom_segment(aes(x = 0, xend = score, yend = label), color = project_palette[["medium_grey"]], linewidth = 0.7) +
