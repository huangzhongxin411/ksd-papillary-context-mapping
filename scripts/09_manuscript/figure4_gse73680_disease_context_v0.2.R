...
panel_c <- ggplot(mod_plot, aes(paired_delta, module_name, fill = signal_class)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "#999999", linewidth = 0.3) +
  geom_col(width = 0.64, color = "#666666", linewidth = 0.2) +
  geom_text(aes(label = fdr_label), hjust = 1.05, size = 2.35, color = pal$text) +
  scale_fill_manual(values = c("FDR < 0.05" = pal$strong, Nominal = pal$warn, "No detectable" = pal$muted)) +
...
ggsave("results/figures/figure4_gse73680_disease_context_v0.2.pdf", fig, width = 12.5, height = 8.8, units = "in", device = "pdf")
ggsave("results/figures/figure4_gse73680_disease_context_v0.2.png", fig, width = 12.5, height = 8.8, units = "in", dpi = 240)

writeLines(c(
  "# Figure 4 Legend v0.2",
  "",
...
), "docs/figure4_legend_v0.2.md")

message("wrote Figure 4 v0.2")
