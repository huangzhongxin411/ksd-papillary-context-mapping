...
version <- "v1.0.1"
version_dir <- "results/figures/figure3_revisions/v1.0.1_p1_gene_evidence_20260621"
dir.create(version_dir, recursive = TRUE, showWarnings = FALSE)
stem <- file.path(version_dir, "figure3_p1_gene_evidence_v1.0.1")
legend_path <- file.path(version_dir, "figure3_legend_v1.0.1.md")
notes_path <- file.path(version_dir, "figure3_revision_notes_v1.0.1.md")
source_path <- file.path(version_dir, "figure3_panel_source_files_v1.0.1.tsv")
qc_path <- file.path(version_dir, "figure3_visual_qc_v1.0.1.tsv")
...
plot_expression_fingerprint <- function() {
  ...
  ggplot(d, aes(context, gene)) +
    ...
    theme(axis.text.x = element_text(angle = 40, hjust = 1), axis.text.y = element_text(face = "italic"),
      ...
      legend.position = "bottom", legend.box = "vertical", legend.spacing.y = unit(0.01, "cm"),
...
plot_evidence_glyph_matrix <- function() {
  ...
  ggplot() +
    ...
    geom_text(data = d, aes(6, y, label = c("TAL id.", "Transport", "Ion", "Ca sense", "Support", "Broad epi.")),
    ...
    labs(title = "D. Gene-centric evidence\nsummary",
      subtitle = "Bulk single-gene validation is not established") +
    theme_void(base_family = "sans") +
    theme(plot.title = element_text(size = 8.8, lineheight = 0.95, face = "bold", color = pal[["text"]], margin = margin(b = 3)),
      plot.subtitle = element_text(size = 6.4, color = pal[["bluegrey"]], margin = margin(b = 4)),
...
notes_text <- paste0(
  "# Figure 3 revision notes (v1.0.1)\n\n",
  ...
  "12. v1.0.1 preserves v1.0 and repairs bottom-row title collision, widens Panel D and shortens matrix role labels. Remaining manual review: inspect at final journal placement size.\n")
...
message("Figure 3 v1.0.1 written to: ", version_dir)
