...
version_dir <- "results/figures/figure4_revisions/v1.0.1_gse73680_disease_context_20260621"
dir.create(version_dir, recursive = TRUE, showWarnings = FALSE)
stem <- file.path(version_dir, "figure4_gse73680_disease_context_v1.0.1")
legend_path <- file.path(version_dir, "figure4_legend_v1.0.1.md")
notes_path <- file.path(version_dir, "figure4_revision_notes_v1.0.1.md")
source_path <- file.path(version_dir, "figure4_panel_source_files_v1.0.1.tsv")
qc_path <- file.path(version_dir, "figure4_visual_qc_v1.0.1.tsv")
...
p_a <- ggplot() +
  ...
  annotate("segment", x = 3.5, xend = 3.5, y = 1.23, yend = 1.08, color = pal[["bluegrey"]],
    linewidth = 0.6, arrow = arrow(length = unit(0.06, "in"))) +
  annotate("segment", x = 0.38, xend = 0.92, y = 2.03, yend = 1.63, color = pal[["mid_grey"]], linewidth = 1.0) +
  annotate("point", x = 0.38, y = 2.03, size = 2.8, shape = 21, fill = pal[["bluegrey"]], color = "white") +
  annotate("point", x = 0.92, y = 1.63, size = 2.8, shape = 21, fill = pal[["terracotta"]], color = "white") +
  annotate("text", x = 0.65, y = 1.30, label = "paired", size = 2.0, color = pal[["bluegrey"]]) +
...
p_d <- ggplot(bm, aes(percentile, module_label)) +
  ...
  annotate("label", x = 0.08, y = 0.05, hjust = 0, vjust = 0,
    ...
    linewidth = 0, label.padding = unit(0.18, "lines")) +
  ...
  labs(title = "D. Size-matched benchmark\nand robustness",
    subtitle = "Teal = exceeds 95th percentile; grey = background-like",
    x = "Size-matched random-set percentile", y = NULL) +
  theme_f4(8.0) +
  theme(axis.text.y = element_text(size = 7.0), axis.line.y = element_blank(), axis.ticks.y = element_blank(),
    plot.title = element_text(size = 9.3, lineheight = 0.95), plot.subtitle = element_text(size = 6.5),
...
writeLines(c("# Figure 4 legend (v1.0.1)", "",
...
writeLines(c("# Figure 4 revision notes (v1.0.1)", "",
  ...
  "8. v1.0.1 preserves v1.0 and repairs Panel A text crowding plus Panel D title, axis and legend clipping."), notes_path)
...
message("Figure 4 v1.0.1 written to: ", version_dir)
