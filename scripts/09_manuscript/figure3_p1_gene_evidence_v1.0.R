...
version <- "v1.0"
version_dir <- "results/figures/figure3_revisions/v1.0_p1_gene_evidence_20260621"
dir.create(version_dir, recursive = TRUE, showWarnings = FALSE)
stem <- file.path(version_dir, "figure3_p1_gene_evidence_v1.0")
legend_path <- file.path(version_dir, "figure3_legend_v1.0.md")
notes_path <- file.path(version_dir, "figure3_revision_notes_v1.0.md")
source_path <- file.path(version_dir, "figure3_panel_source_files_v1.0.tsv")
qc_path <- file.path(version_dir, "figure3_visual_qc_v1.0.tsv")
...
plot_role_spectrum <- function() {
  ...
  ggplot(d, aes(x, y)) +
    ...
    geom_segment(aes(x = 1, xend = 6, y = 1, yend = 1), color = pal[["bluegrey"]], linewidth = 1.0) +
    geom_point(aes(fill = fill), shape = 21, size = 8.2, color = "white", stroke = 1.1,
      show.legend = FALSE) +
    scale_fill_identity() +
    geom_text(aes(label = gene), y = 1.04, color = "white", fontface = "bold.italic", size = 2.65) +
...
plot_expression_fingerprint <- function() {
  ...
  br <- unique(quantile(d$avg_expression, probs = seq(0, 1, length.out = 6), na.rm = TRUE))
  if (length(br) < 6) br <- seq(min(d$avg_expression), max(d$avg_expression), length.out = 6)
  d[, expression_bin := cut(avg_expression, breaks = br, include.lowest = TRUE,
    labels = c("Very low", "Low", "Moderate", "High", "Highest"))]
  ggplot(d, aes(context, gene)) +
    ...
    scale_fill_manual(values = c("Very low" = pal[["pale_grey"]], "Low" = pal[["pale_bluegrey"]],
      "Moderate" = "#A9C1C6", "High" = pal[["bluegrey"]], "Highest" = pal[["deep_teal"]]),
      name = "Mean expression", drop = FALSE) +
    scale_size_continuous(range = c(1.1, 6.0), breaks = c(25, 50, 75), limits = c(0, 100),
      name = "Detected nuclei (%)") +
    labs(title = "B. P1 expression across audited contexts",
      subtitle = "Dot size = detected nuclei; color = mean expression", x = NULL, y = NULL) +
    theme_f3(8.0) +
    theme(axis.text.x = element_text(angle = 40, hjust = 1), axis.text.y = element_text(face = "italic"),
      ...
      legend.position = "bottom", legend.box = "vertical", legend.spacing.y = unit(0.02, "cm"),
...
plot_specificity_lollipop <- function() {
  ...
  ggplot(d, aes(log2_ratio, gene)) +
    ...
    geom_text(aes(label = sprintf("%.1f", specificity_ratio_avg)), nudge_x = 0.13,
      hjust = 0, size = 2.1, color = pal[["text"]]) +
    ...
    labs(title = "C. TAL specificity and donor support",
      subtitle = "Descriptive single-nucleus evidence; not disease validation",
...
plot_evidence_glyph_matrix <- function() {
  ...
  ggplot() +
    ...
    geom_text(data = d, aes(6, y, label = unname(roles[gene])), size = 1.85, color = pal[["text"]]) +
    ...
    labs(title = "D. Gene-centric evidence summary",
      subtitle = "Glyph summary; bulk single-gene validation is not established") +
    theme_void(base_family = "sans") +
    theme(plot.title = element_text(size = 9.6, face = "bold", color = pal[["text"]], margin = margin(b = 3)),
      plot.subtitle = element_text(size = 7.0, color = pal[["bluegrey"]], margin = margin(b = 4)),
...
bottom <- plot_grid(p_b, p_c, p_d, ncol = 3, rel_widths = c(0.36, 0.28, 0.36), align = "h", axis = "tb")
...
notes_text <- paste0(
  ...
  "12. Remaining manual review: inspect gene and matrix labels at final journal placement size; no biological content changes are recommended.\n")
...
message("Figure 3 v1.0 written to: ", version_dir)
