...
version_dir <- "results/figures/figure4_revisions/v1.1_gse73680_disease_context_20260621"
dir.create(version_dir, recursive = TRUE, showWarnings = FALSE)
stem <- file.path(version_dir, "figure4_gse73680_disease_context_v1.1")
legend_path <- file.path(version_dir, "figure4_legend_v1.1.md")
notes_path <- file.path(version_dir, "figure4_revision_notes_v1.1.md")
source_path <- file.path(version_dir, "figure4_panel_source_files_v1.1.tsv")
qc_path <- file.path(version_dir, "figure4_visual_qc_v1.1.tsv")
...
p_b <- ggplot(p1, aes(paired_delta, gene)) +
  ...
  scale_x_continuous(limits = c(-0.60, 0.98), breaks = c(-0.5, 0, 0.5), expand = expansion(mult = c(0, 0.02))) +
...
p_c <- ggplot(sc, aes(condition, patient_level_module_score, group = patient_id)) +
  ...
  stat_summary(aes(group = 1), fun = mean, geom = "line", color = pal[["deep_teal"]], linewidth = 1.15) +
  stat_summary(aes(group = 1), fun = mean, geom = "point", color = pal[["deep_teal"]], size = 2.0) +
...
p_d <- ggplot(bm, aes(percentile, module_label)) +
  ...
  labs(title = "D. Size-matched benchmark\nand robustness",
    subtitle = "Teal: >95th percentile\nGrey: background-like",
...
note_d <- ggdraw() +
  draw_label(sprintf("Robustness: LOO direction retained %.0f%%; without P1 q=%.3f", loo_direction * 100, without_p1_q),
    x = 0.03, hjust = 0, size = 6.4, color = pal[["deep_teal"]]) +
  theme(plot.background = element_rect(fill = pal[["very_pale"]], color = NA))
p_d2 <- plot_grid(p_d, note_d, ncol = 1, rel_heights = c(0.91, 0.09))
bottom <- plot_grid(p_c, p_d2, ncol = 2, rel_widths = c(0.62, 0.38), align = "h")
...
writeLines(c("# Figure 4 legend (v1.0.2)", "",
...
writeLines(c("# Figure 4 revision notes (v1.0.2)", "",
  ...
  "7. Figure 2/Figure 3 typography, semantic palette, white background and minimal-grid style were applied.",
  "8. v1.0.2 preserves prior versions and shortens Panel D subtitle and axis title to prevent narrow-column clipping."), notes_path)
...
message("Figure 4 v1.1 written to: ", version_dir)
