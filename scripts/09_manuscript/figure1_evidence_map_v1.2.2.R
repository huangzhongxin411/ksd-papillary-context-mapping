...
version_dir <- "results/figures/figure1_revisions/v1.2.2_card_refinement_20260621"
...
draw_context <- function(x, y, w, h) {
  ...
  txt("Plaque/stone", x + w * 0.71, y + h * 0.12, size = 8.0, colr = col$terracotta, font = 2, just = "left")
}
...
draw_boundary <- function(x, y, w, h) {
  ...
  draw_check(x + w * 0.51, y + h - 0.150, col$terracotta, "SMR/coloc support", "cross")
  ...
  txt("Resource-limited extensions are audited but not used as evidence layers.",
    x + 0.030, y + 0.013, size = 8.0, colr = "#657277", just = "left")
}
...
draw_figure <- function() {
  ...
  txt("Module activity shifts in\nplaque/stone papilla\nversus matched controls", xs[4] + card_w / 2,
    card_y + 0.057, size = 8.5, colr = col$dark, lineheight = 0.94)
...
base <- file.path(version_dir, "figure1_evidence_map_v1.2.2")
save_all(base)

writeLines(c(
  "# Figure 1 Legend v1.2.2", "",
...
), file.path(version_dir, "figure1_legend_v1.2.2.md"), useBytes = TRUE)
...
source_dt <- data.table(
  ...
  source_file = c(
    "scripts/09_manuscript/figure1_evidence_map_v1.2.2.R",
...
fwrite(source_dt, file.path(version_dir, "figure1_source_files_v1.2.2.tsv"), sep = "\t")
...
fwrite(qc, file.path(version_dir, "figure1_visual_qc_v1.2.2.tsv"), sep = "\t")

writeLines(c(
  "# Figure 1 Revision Notes v1.2.2", "",
  "## 1. Changes from v1.1", "",
...
), file.path(version_dir, "figure1_revision_notes_v1.2.2.md"), useBytes = TRUE)

message("Figure 1 v1.2.2 revision folder written: ", version_dir)
