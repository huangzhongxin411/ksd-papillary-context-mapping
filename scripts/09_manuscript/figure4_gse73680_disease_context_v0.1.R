...
panel_b <- ggplot(p1, aes(paired_delta, gene, fill = signal_class)) +
  ...
  geom_text(aes(label = sprintf("P=%.3f", p_value)), hjust = ifelse(p1$paired_delta >= 0, -0.05, 1.05), size = 2.5) +
  ...
  labs(x = "Patient-level paired delta", y = NULL, fill = "Signal",
       title = "B. P1 genes show heterogeneous disease-context response") +
  theme_bw(base_size = 9.5) +
...
panel_c <- ggplot(mod_plot, aes(paired_delta, module_name, fill = signal_class)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "#999999", linewidth = 0.3) +
  geom_col(width = 0.64, color = "#666666", linewidth = 0.2) +
  geom_text(aes(label = sprintf("FDR=%.3f", fdr)), hjust = -0.05, size = 2.45) +
  scale_fill_manual(values = c("FDR < 0.05" = pal$strong, Nominal = pal$warn, "No detectable" = pal$muted)) +
  labs(x = "Patient-level paired module delta", y = NULL, fill = "Signal",
       title = "C. MAGMA modules increase in plaque/stone papilla context") +
  theme_bw(base_size = 9.5) +
...
panel_d <- ggplot(bench_plot, aes(percentile, module_name, fill = benchmark_class)) +
  ...
  geom_col(width = 0.64, color = "#666666", linewidth = 0.2) +
  geom_text(aes(label = sprintf("emp.P=%.3f", empirical_p)), hjust = -0.05, size = 2.45) +
...
ggsave("results/figures/figure4_gse73680_disease_context_v0.1.pdf", fig, width = 12.5, height = 8.8, units = "in", device = "pdf")
ggsave("results/figures/figure4_gse73680_disease_context_v0.1.png", fig, width = 12.5, height = 8.8, units = "in", dpi = 240)

writeLines(c(
  "# Figure 4 Legend v0.1",
  "",
...
), "docs/figure4_legend_v0.1.md")

message("wrote Figure 4 v0.1")
