...
panel_c <- ggplot(donor_tal, aes(gene, avg_expression)) +
  ...
  geom_point(aes(color = disease_status), position = position_jitter(width = 0.09, height = 0),
             size = 2.95, alpha = 0.98) +
  scale_color_manual(values = c(healthy_control = pal$healthy, stone_disease = pal$stone), drop = FALSE) +
...
matrix_dt[, donor_support := ifelse(TAL_donor_detection_fraction >= 0.75, "3/4", "<3/4")]
...
fill_values <- c(
  ...
  "+" = pal$strong, weak = "#B59A5B", "-" = pal$negative,
...
ggsave("results/figures/figure3_p1_gene_evidence_v0.3.pdf", fig, width = 12.5, height = 8.8, units = "in", device = "pdf")
ggsave("results/figures/figure3_p1_gene_evidence_v0.3.png", fig, width = 12.5, height = 8.8, units = "in", dpi = 240)

writeLines(c(
  "# Figure 3 Legend v0.3",
  "",
  ...
  "(C) Donor-level TAL expression of representative P1 genes. Each point represents one donor-level average and is shown for descriptive consistency rather than inferential disease comparison.",
  "(D) Gene-centric evidence matrix summarizing MAGMA support, TAL expression rank, donor-level detection, TAL specificity, TAL program consistency and manuscript role assignment.",
...
writeLines(c(
  "# Results Draft v0.4",
  "",
  "> Current evidence grade: MAGMA + scRNA-supported TAL-associated cellular context. Result 4 is reserved for GSE73680 only if analysis-ready criteria are met.",
  "",
  ...
  "Six P1 core candidate genes were evaluated in the audited GSE231569 single-nucleus dataset. The genes did not behave as a uniform TAL-specific marker set. Instead, UMOD represented a TAL-associated gene, CLDN10 supported an epithelial transport pattern, CLDN14 and CASR linked the signal to ion-handling and calcium-sensing biology, HIBADH remained a supporting MAGMA-associated context gene and PKD2 represented a broader renal epithelial context. Figure 3 v0.3 summarizes this evidence across expression, specificity, donor-level consistency and gene-role assignment.",
  "",
  "## Result 4. Disease-context evaluation in GSE73680",
  "Reserved. GSE73680 is currently a resource-landing and analysis-readiness module. It must not be written as a disease-context result until a readable expression matrix, reliable curated metadata, gene mapping, expression QC and gene-set availability checks pass the analysis-ready criteria.",
  "",
  "## Boundary",
  "The current manuscript-ready claim remains MAGMA + scRNA-supported TAL-associated cellular context. TWAS, SMR/coloc, spatial and GSE73680 disease-context analyses remain pending/resource-limited unless their required resources become available and pass QC."
), "docs/results_draft_v0.4.md")

message("wrote Figure 3 v0.3 and Results v0.4")
