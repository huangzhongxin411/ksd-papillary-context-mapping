...
make_figure2_v09 <- function() {
  ...
  p_a <- ggplot() +
    ...
    labs(title = "C. Audited GSE231569 snRNA-seq atlas", x = "UMAP 1", y = "UMAP 2") +
    theme_hi(9) +
  ...
  p_c <- ggplot(bench, aes(cell_label, module_label, fill = benchmark_percentile)) +
    ...
    scale_fill_gradientn(colors = c(hi_pal$light_grey, hi_pal$bluegrey, hi_pal$deep_teal), limits = c(0, 1), name = "Benchmark\npercentile") +
    labs(title = "D. Cell-type benchmark percentile", x = NULL, y = NULL) +
    theme_hi(8.8) +
  ...
  p_d <- ggplot(infl, aes(contribution_score, gene)) +
    ...
    labs(title = "E. Leading contributors to Loop/TAL-associated signal", x = "Contribution score", y = NULL, fill = NULL) +
    theme_hi(9) + theme(axis.text.y = element_text(face = "italic"), legend.position = "bottom")

  lower <- plot_grid(p_b, p_a, p_c, p_d, ncol = 2, rel_heights = c(1.08, 0.92))
  fig <- plot_grid(manhattan, lower, ncol = 1, rel_heights = c(0.72, 1.70))
  hi_save_fig(fig, "results/figures/figure2_magma_scrna_localization_final_candidate", 13.2, 12.2)
  hi_write_lines(c("# Figure 2 Legend Final Candidate", "",
    "**Figure 2. GWAS-prioritized KSD genes localize to a Loop/TAL-associated renal papillary single-nucleus context.**",
    "(A) Manhattan plot of the cleaned 2025 trans-ancestry KSD GWAS summary statistics. The dashed line denotes P = 5 x 10^-8; selected loci corresponding to downstream-prioritized or biologically interpretable genes are annotated without implying causal assignment. (B) MAGMA top 50 module score projected onto the GSE231569 UMAP with a continuous grey-to-teal scale and Loop/TAL density outline. (C) Audited GSE231569 snRNA-seq atlas with Loop/TAL cells highlighted over low-opacity audited compartments. (D) Size-matched random benchmark percentile across audited cell contexts. (E) Ranked display of leading Loop/TAL-associated contributors. Contribution score is descriptive and is not causal driver inference."),
    "docs/figure2_legend_final_candidate.md")
  write_source_table("figure2", data.table(
    panel = c("A", "B-C", "D", "E"),
    source_file = c("data/processed/gwas/2025_trans_ancestry/meta_sumstats.cleaned.tsv.gz", "results/tables/gse231569_celllevel_magma_scores.tsv", "results/tables/magma_scrna_random_benchmark.tsv", "results/tables/loop_tal_influential_magma_genes.tsv"),
    role = c("GWAS association landscape and selected representative loci", "UMAP coordinates and cell-level module scores", "cell-type benchmark percentiles", "ranked contributor display")
  ))
  write_visual_qc("figure2", "final_candidate", data.table(check = c("real_gwas_manhattan", "manhattan_rasterization", "umap_loop_tal_highlight", "score_colorbar", "heatmap_numeric_cells", "lollipop_detection_legend", "claim_boundary"),
    status = "pass", note = c("cleaned 2025 trans-ancestry GWAS with seven labeled loci", "dense points rasterized; labels and axes remain vector",
    "Loop/TAL highlighted with contour and label n=540", "colorbar labelled Module score",
    "every benchmark cell has a two-decimal percentile", "detection legend simplified to 20/50/80", "loci and contributors are descriptive, not causal")))
}
...
make_figure3_v11 <- function() {
  segments <- data.table(
    ...
    fill = c(hi_pal$loop_tal, hi_pal$bluegrey, hi_pal$sand, hi_pal$bluegrey, hi_pal$terracotta)
  )
  pills <- data.table(
  ...
  p_b <- ggplot(cell, aes(cell_label, gene)) +
    ...
    scale_fill_gradientn(colors = c(hi_pal$light_grey, hi_pal$bluegrey, hi_pal$deep_teal), name = "Average\nexpression") +
  ...
  p_d <- ggplot() +
    geom_tile(data = ev_long, aes(evidence, gene), fill = hi_pal$light_grey, color = "white", linewidth = 0.6) +
  ...
  hi_save_fig(fig, "results/figures/figure3_p1_gene_evidence_final_candidate", 13.2, 8.9)
  hi_write_lines(c("# Figure 3 Legend Final Candidate", "",
    ...
    "docs/figure3_legend_final_candidate.md")
  ...
  write_visual_qc("figure3", "final_candidate", data.table(check = c("role_ribbon", "gene_order", "loop_tal_background", "glyph_matrix", "bulk_boundary"),
...
make_figure4_v14 <- function() {
  ...
  p_b <- ggplot(p1, aes(paired_delta, gene, fill = signal_class)) +
    ...
    scale_fill_manual(values = c("PKD2 nominal only" = hi_pal$sand, "No FDR support" = hi_pal$light_grey)) +
  ...
  p_c <- ggplot(long, aes(group_label, patient_level_module_score, group = patient_id)) +
    ...
    scale_fill_manual(values = c("Control/adjacent" = hi_pal$bluegrey, "Plaque/stone papilla" = hi_pal$terracotta)) +
  ...
  p_d <- ggplot(bench, aes(percentile, module_label)) +
    ...
    scale_fill_manual(values = c("TRUE" = hi_pal$deep_teal, "FALSE" = hi_pal$light_grey), labels = c("P1 boundary", "MAGMA module"), name = NULL) +
  ...
  hi_save_fig(fig, "results/figures/figure4_gse73680_disease_context_final_candidate", 13.2, 9.3)
  hi_write_lines(c("# Figure 4 Legend Final Candidate", "",
    ...
    "docs/figure4_legend_final_candidate.md")
  ...
  write_visual_qc("figure4", "final_candidate", data.table(check = c("compact_design", "p1_boundary", "module_shift_labels", "benchmark_lollipop", "expression_matched_caption"),
...
make_figure5_v06 <- function() {
  ...
  p_go <- ggplot(go, aes(-log10(p.adjust), short, size = Count, fill = gene_set_label)) +
    ...
    scale_fill_manual(values = c("MAGMA top 100" = hi_pal$deep_teal, "MAGMA FDR" = hi_pal$bluegrey,
    ...
    theme_hi(8.7) + theme(legend.position = "bottom", legend.box = "vertical",
  ...
  p_cur <- ggplot(curated, aes(gene_set_label, term_label, fill = fill_cap)) +
    ...
    scale_color_identity() +
    scale_fill_gradient(low = hi_pal$light_grey, high = hi_pal$deep_teal) +
  ...
  p_heat <- ggplot(d, aes(injury_label, module_label, fill = pmin(pmax(rho, 0), 1))) +
    ...
    scale_fill_gradient(low = hi_pal$light_grey, high = hi_pal$terracotta, limits = c(0, 1), name = "Spearman\nrho") +
  ...
  hi_save_fig(fig, "results/figures/figure5_functional_context_final_candidate", 13.2, 8.9)
  hi_write_lines(c("# Figure 5 Legend Final Candidate", "",
    ...
    "docs/figure5_legend_final_candidate.md")
  ...
  write_visual_qc("figure5", "final_candidate", data.table(check = c("two_column_layout", "go_short_labels", "curated_white_text", "risk_injury_large_panel", "p1_boundary"),
...
if (figure_only %in% c("all", "figure2")) make_figure2_v09()
if (figure_only == "all") {
  make_figure3_v11()
  make_figure4_v14()
  make_figure5_v06()
...
final_stems <- c(
  "figure1_evidence_map_final_candidate",
  "figure2_magma_scrna_localization_final_candidate",
  "figure3_p1_gene_evidence_final_candidate",
  "figure4_gse73680_disease_context_final_candidate",
  "figure5_functional_context_final_candidate"
)
final_dir <- "results/figures/final_main_figures_candidate"
dir.create(final_dir, recursive = TRUE, showWarnings = FALSE)
stems_to_copy <- if (figure_only == "figure2") final_stems[2] else final_stems
for (stem in stems_to_copy) {
  for (ext in c("pdf", "png", "svg")) {
    src <- file.path("results/figures", paste0(stem, ".", ext))
    if (file.exists(src)) file.copy(src, file.path(final_dir, basename(src)), overwrite = TRUE)
  }
}

qc <- data.table(
  figure_id = paste0("Figure ", 1:5),
  version = "final_candidate",
  output_pdf = file.path("results/figures", paste0(final_stems, ".pdf")),
  output_png = file.path("results/figures", paste0(final_stems, ".png")),
  output_svg = file.path("results/figures", paste0(final_stems, ".svg")),
  pdf_exists = file.exists(file.path("results/figures", paste0(final_stems, ".pdf"))),
  png_exists = file.exists(file.path("results/figures", paste0(final_stems, ".png"))),
  svg_exists = file.exists(file.path("results/figures", paste0(final_stems, ".svg"))),
  font_family = "sans",
  panel_labels = c("not_applicable_framework", "A/B/C/D/E bold upper-left", rep("A/B/C/D bold upper-left", 2), "A/B/C bold upper-left"),
  palette = "deep teal / blue-grey / sand gold / terracotta / light grey / dark text",
  legend_strategy = c("direct card headers", "bottom/right", "bottom", "bottom", "bottom/right"),
  svg_embedded_raster = c("none", "Manhattan dense point layer plus gradient legend", "gradient_legend_only", "none", "gradient_legend_only"),
  svg_core_data_layers_vector = "pass",
  manhattan_source = c("real cleaned GWAS + all Phase 1 lead SNPs", "real cleaned GWAS + Phase 1 labeled loci", rep("not_applicable", 3)),
  claim_boundary = c(
    "framework only; no causality/TWAS/SMR-coloc/spatial/P1 validation",
    "Loop/TAL-associated localization; not causal cell-type assignment",
    "P1 expression context; not single-gene disease validation",
    "module-level disease-context association; not single-gene validation",
    "functional/injury coupling; not mechanism or pathway validation"
  ),
  visual_status = "pass",
...
hi_write_lines(c(
  "# Main Figure Style Guide - Final Candidate", "",
  "Figure 1 is the visual mother template for Figures 2-5.", "",
  "## Palette",
  "- Deep teal `#0F5A64`: genetic signal and Loop/TAL focal evidence.",
  "- Blue grey `#6F929B`: other audited cellular context.",
  "- Sand gold `#C49A3A`: P1 candidate interpretation.",
  "- Terracotta `#A75A49`: plaque/stone disease context and injury coupling.",
  "- Light grey `#EEF2F3`: background and no-support states.",
  "- Dark text `#243038`: typography and axes.", "",
  "## Layout",
  "Panel labels are bold A/B/C/D titles at the upper left. Legends sit below or to the right of data panels. Explanatory prose is retained in figure legends rather than overlaid on dense data panels.", "",
  "## Claim Lock",
  "The figures support Loop/TAL-associated cellular localization, P1 interpretive context, MAGMA module-level disease-context association and functional/injury coupling. They do not establish causality, causal mediation, TWAS convergence, SMR/coloc support, spatial validation, P1 disease-gene validation, pathway activity validation or a confirmed mechanism."
), "docs/main_figure_style_guide_final_candidate.md")

message("Phase 17C-B final-candidate Figure 2-5 harmonization outputs written")
