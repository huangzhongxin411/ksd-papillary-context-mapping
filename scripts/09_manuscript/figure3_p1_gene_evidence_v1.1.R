...
version <- "v1.1"
version_dir <- "results/figures/figure3_revisions/v1.1_p1_gene_evidence_20260621"
dir.create(version_dir, recursive = TRUE, showWarnings = FALSE)
stem <- file.path(version_dir, "figure3_p1_gene_evidence_v1.1")
...
plot_specificity_lollipop <- function() {
  ...
  ggplot(d, aes(log2_ratio, gene)) +
    ...
    geom_text(aes(label = sprintf("%.1f\u00d7", specificity_ratio_avg)), nudge_x = -0.15,
      hjust = 1, size = 2.25, color = pal[["text"]]) +
