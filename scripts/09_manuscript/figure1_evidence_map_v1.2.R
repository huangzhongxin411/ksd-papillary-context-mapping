...
version_dir <- "results/figures/figure1_revisions/v1.2_card_refinement_20260621"
...
draw_figure <- function() {
  ...
  txt("Post-GWAS evidence map for kidney stone papillary cellular context",
    0.5, 0.942, size = 18.5, colr = col$dark, font = 2)
  txt("Genetic prioritization, single-nucleus localization, candidate-gene interpretation and plaque/stone disease-context association",
    0.5, 0.902, size = 8.6, colr = "#58666D")
  ...
  for (i in 1:4) {
    ...
    txt(headers[i], xs[i] + 0.033, 0.810, size = 10.8, colr = col$dark, font = 2, just = "left")
    txt(subs[i], xs[i] + 0.033, 0.786, size = 8.5, colr = col$dark, just = "left")
...
base <- file.path(version_dir, "figure1_evidence_map_v1.2")
save_all(base)

writeLines(c(
  "# Figure 1 Legend v1.2", "",
...
), file.path(version_dir, "figure1_legend_v1.2.md"), useBytes = TRUE)
...
source_dt <- data.table(
  ...
  source_file = c(
    "scripts/09_manuscript/figure1_evidence_map_v1.2.R",
...
fwrite(source_dt, file.path(version_dir, "figure1_source_files_v1.2.tsv"), sep = "\t")
...
fwrite(qc, file.path(version_dir, "figure1_visual_qc_v1.2.tsv"), sep = "\t")

writeLines(c(
  "# Figure 1 Revision Notes v1.2", "",
  "## 1. Changes from v1.1", "",
  "The preferred v1.1 four-card composition was retained. The new version uses a double-column 180 x 115 mm canvas, 600 dpi PNG export, exact muted project colors, lighter card borders and a simplified internal visual hierarchy.",
  "",
...
), file.path(version_dir, "figure1_revision_notes_v1.2.md"), useBytes = TRUE)

message("Figure 1 v1.2 revision folder written: ", version_dir)
