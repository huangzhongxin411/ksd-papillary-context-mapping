...
version_dir <- "results/figures/figure2_revisions/v1.0.1_gwas_magma_scrna_20260621"
dir.create(version_dir, recursive = TRUE, showWarnings = FALSE)

stem <- file.path(version_dir, "figure2_gwas_magma_scrna_localization_v1.0.1")
legend_path <- file.path(version_dir, "figure2_legend_v1.0.1.md")
notes_path <- file.path(version_dir, "figure2_revision_notes_v1.0.1.md")
source_path <- file.path(version_dir, "figure2_panel_source_files_v1.0.1.tsv")
qc_path <- file.path(version_dir, "figure2_visual_qc_v1.0.1.tsv")
...
p_b <- ggplot(top50, aes(UMAP_1, UMAP_2)) +
  ...
  scale_color_stepsn(colors = c(pal[["point_grey"]], pal[["light_blue"]], pal[["bluegrey"]], pal[["deep_teal"]]),
    limits = score_limits, oob = squish, breaks = score_limits, labels = c("Low", "High"),
    name = "Relative score",
    guide = guide_colorsteps(barwidth = unit(0.70, "in"), barheight = unit(0.08, "in"), title.position = "top")) +
  labs(title = "B. MAGMA top50 score\non audited UMAP", x = "UMAP 1", y = "UMAP 2") +
...
p_c <- ggplot(bench, aes(benchmark_percentile, cell_label)) +
  ...
  labs(title = "C. Size-matched benchmark",
...
p_d <- ggplot(contributors, aes(contribution_score, gene)) +
  ...
  labs(title = "D. Leading Loop/TAL contributors",
...
writeLines(c(
  "# Figure 2 Revision Notes v1.0.1", "",
...
message("Figure 2 v1.0.1 revision folder written: ", version_dir)
