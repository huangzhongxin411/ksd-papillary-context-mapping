...
version_dir <- "results/figures/figure2_revisions/v1.0_gwas_magma_scrna_20260621"
dir.create(version_dir, recursive = TRUE, showWarnings = FALSE)

stem <- file.path(version_dir, "figure2_gwas_magma_scrna_localization_v1.0")
legend_path <- file.path(version_dir, "figure2_legend_v1.0.md")
notes_path <- file.path(version_dir, "figure2_revision_notes_v1.0.md")
source_path <- file.path(version_dir, "figure2_panel_source_files_v1.0.tsv")
qc_path <- file.path(version_dir, "figure2_visual_qc_v1.0.tsv")
...
theme_f2 <- function(base_size = 8.0) {
  theme_classic(base_size = base_size, base_family = "sans") +
    theme(
      plot.title = element_text(size = 10.5, face = "bold", color = pal[["text"]], margin = margin(b = 3)),
...
p_b <- ggplot(top50, aes(UMAP_1, UMAP_2)) +
  ...
  annotate("label", x = tal_center$x + 3.0, y = tal_center$y + 3.3,
    label = "Loop/TAL\nn = 540", hjust = 0, size = 2.35, fontface = "bold",
    color = pal[["deep_teal"]], fill = alpha("white", 0.90), linewidth = 0.18) +
  scale_color_stepsn(colors = c(pal[["point_grey"]], pal[["light_blue"]], pal[["bluegrey"]], pal[["deep_teal"]]),
    limits = score_limits, oob = squish, n.breaks = 4, name = "Relative\nmodule score",
    guide = guide_colorsteps(barwidth = unit(0.32, "in"), barheight = unit(0.09, "in"), title.position = "top")) +
  labs(title = "B. MAGMA top50 score localizes\nto Loop/TAL cells", x = "UMAP 1", y = "UMAP 2") +
  theme_f2(8.0) + theme(legend.position = "bottom", legend.direction = "horizontal")
...
cell_order <- c("Loop/TAL", "Perivascular/mural-like", "Injured epithelial",
  "Fibroblast/stromal", "Endothelial", "Collecting duct")
bench[, cell_label := factor(label_cell(audited_broad_cell_type), levels = rev(cell_order))]
...
p_c <- ggplot(bench, aes(benchmark_percentile, cell_label)) +
  ...
  labs(title = "C. Loop/TAL exceeds size-matched\nrandom expectation",
    subtitle = "Dashed line, 95th percentile; low-abundance contexts interpreted cautiously",
...
p_d <- ggplot(contributors, aes(contribution_score, gene)) +
  ...
  labs(title = "D. Leading contributors to the\nLoop/TAL-associated signal",
...
bottom <- plot_grid(p_b, p_c, p_d, ncol = 3, rel_widths = c(0.32, 0.34, 0.34), align = "h", axis = "tb")
...
message("Figure 2 v1.0 revision folder written: ", version_dir)
