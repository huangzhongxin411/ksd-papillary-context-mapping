...
version_dir <- "results/figures/figure2_revisions/20260622_highimpact_polish"
dir.create(version_dir, recursive = TRUE, showWarnings = FALSE)

stem <- file.path(version_dir, "figure2_highimpact_polish")
legend_path <- file.path(version_dir, "figure2_legend_highimpact_polish.md")
notes_path <- file.path(version_dir, "figure2_revision_notes_highimpact_polish.md")
source_path <- file.path(version_dir, "figure2_panel_source_files_highimpact_polish.tsv")
qc_path <- file.path(version_dir, "figure2_visual_qc_highimpact_polish.tsv")

pal <- c(
  deep_teal = "#005A64", bluegrey = "#6F929B", sand = "#C99A2E",
  terracotta = "#A85B4B", cool_grey = "#EAF1F2", border = "#D3DADC",
  text = "#22313B", point_grey = "#D3DADC", mid_grey = "#6F929B",
  light_blue = "#B7C8CD", white = "#FFFFFF"
)

theme_f2 <- function(base_size = 9.0) {
  theme_classic(base_size = base_size, base_family = "Helvetica") +
    theme(
      ...
      strip.text = element_text(size = 9, face = "bold", color = pal[["text"]]),
...
writeLines(c(
  "# Figure 2 Revision Notes v1.0.3", "",
...
message("Figure 2 v1.0.3 revision folder written: ", version_dir)
