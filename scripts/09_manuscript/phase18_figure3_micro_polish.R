...
version <- "v1.1"
version_dir <- "results/figures/figure3_revisions/phase18_micro_polish_20260621"
dir.create(version_dir, recursive = TRUE, showWarnings = FALSE)
stem <- file.path(version_dir, "figure3_p1_gene_evidence_phase18_final")
legend_path <- file.path(version_dir, "figure3_legend_v1.1.md")
notes_path <- file.path(version_dir, "figure3_revision_notes_v1.1.md")
source_path <- file.path(version_dir, "figure3_panel_source_files_v1.1.tsv")
qc_path <- file.path(version_dir, "figure3_visual_qc_v1.1.tsv")
...
theme_f3 <- function(base_size = 8.0) {
  theme_classic(base_size = base_size, base_family = "sans") +
    theme(
      plot.title = element_text(size = 10.2, face = "bold", color = pal[["text"]], margin = margin(b = 3)),
      plot.subtitle = element_text(size = 7.4, color = "#6E929B", margin = margin(b = 4)),
      axis.title = element_text(size = 8.8, color = pal[["text"]]),
      axis.text = element_text(size = 7.6, color = pal[["text"]]),
...
plot_role_spectrum <- function() {
  ...
  ggplot(d, aes(x, y)) +
    ...
    theme(plot.title = element_text(size = 10.2, face = "bold", color = pal[["text"]], margin = margin(b = 2)),
      plot.subtitle = element_text(size = 7.4, color = pal[["bluegrey"]], margin = margin(b = 2)),
...
plot_expression_fingerprint <- function() {
  ...
  ggplot(d, aes(context, gene)) +
    ...
    theme_f3(8.0) +
    theme(axis.text.x = element_text(angle = 28, hjust = 1), axis.text.y = element_text(face = "italic"),
      axis.line = element_blank(), axis.ticks = element_blank(),
      panel.grid.major = element_line(color = pal[["cool_grey"]], linewidth = 0.34),
      plot.title = element_text(size = 9.6, lineheight = 0.95),
      plot.subtitle = element_text(size = 6.7, lineheight = 0.95),
...
plot_specificity_lollipop <- function() {
  ...
  d[, `:=`(label_x = log2_ratio + ifelse(log2_ratio < 0.9, 0.13, -0.13),
    label_hjust = ifelse(log2_ratio < 0.9, 0, 1))]
  ...
  ggplot(d, aes(log2_ratio, gene)) +
    ...
    geom_text(aes(x = label_x, label = sprintf("%.1f\u00d7", specificity_ratio_avg), hjust = label_hjust),
      size = 2.25, color = pal[["text"]]) +
    ...
    theme(axis.text.y = element_text(face = "italic"), axis.line.y = element_blank(), axis.ticks.y = element_blank(),
      ...
      plot.title = element_text(size = 9.6, lineheight = 0.95),
      plot.subtitle = element_text(size = 6.6, lineheight = 0.95),
...
plot_evidence_glyph_matrix <- function() {
  ...
  headers <- c("MAGMA", "TAL", "Donor", "Specificity", "Bulk", "Role")
  ggplot() +
    geom_hline(yintercept = 1.5:6.5, color = pal[["pale_grey"]], linewidth = 0.34) +
    geom_text(data = data.table(x = c(1, 2, 2.85, 4.10, 5.05, 6), label = headers), aes(x, 6.78, label = label),
      size = 1.88, fontface = "bold", color = pal[["text"]]) +
    ...
    theme_void(base_family = "sans") +
    theme(plot.title = element_text(size = 9.6, lineheight = 0.95, face = "bold", color = pal[["text"]], margin = margin(b = 3)),
...
bottom <- plot_grid(p_b, p_c, p_d, ncol = 3, rel_widths = c(0.33, 0.26, 0.41), align = "h", axis = "tb")
